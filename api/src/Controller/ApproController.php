    ...
    public function commands_register(Request $request)
    {
        ...
        for ($i=0; $i<$count; $i++) {
            $var = $em->getConnection()->prepare('CALL __ap_register_prix_moyen2(?, ?, ? ,?, ?,
                                                                          ?, ?, ?, ?, ?);');
            $res = $var->executeQuery([$products[$i]['id'], $agent, $fournisseur, $products[$i]['quantity'],
        ...
        if ($data['credit'] == true) {
            ...
            $res = $var->executeQuery([$data['avance'], 'achat', $id_facture, $this->getUser()->getShopId()]);
        }elseif ($data['credit'] == false) {
            $var = $em->getConnection()->prepare('INSERT INTO s_credit ( montant, type, factureid, date, id_magasin) VALUES (?, ?, ?, NOW(), ?);');
            $res = $var->executeQuery([$data['total'], 'achat', $id_facture, $this->getUser()->getShopId()]);
        }
        return new JsonResponse(['msg'=>'Commande ajoutée avec succès', 'factureid'=> $id_facture, 'status' => 1]);
    }
