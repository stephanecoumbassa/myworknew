...
class DevisController extends AbstractController
{
    ...
    {
        $em = $this->getDoctrine();
        $var = $em->getConnection()->prepare('SELECT distinct(id_vente), client_id, sc.last_name, sc.name,
    ...
    public function devis_venete_get($idvente)
    {
       ...
       credit, codecomptable, status, type, sp.name, client_func_get(client_id, s_devis.id_magasin) client
    ...
    public function devis_register(Request $request)
    {
        ...
        for ($i=0; $i<$count; $i++) {
            ...
            $var = $em->getConnection()->prepare('
                INSERT INTO s_devis (user_id, product_id, quantite_vendu, prix_unitaire, montant_vendu, tva, client_id, id_vente, id_magasin, type) VALUES
                            (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ');
            $var->executeQuery([$agent, $pid , $qty, $sell, ($qty * $sell) , $tva, $clientid, $code_vente, $shop_id, 'devis']);
        }
