...
class PTaskController extends BaseController {
  ...
  public function p_task_get() :JsonResponse
  {
    ...
    $var = $em->getConnection()->prepare("SELECT *,
       (SELECT concat(nom, ' ', prenom) FROM p_employe WHERE p_employe.id = p_task.p_employe_id) employe
       FROM p_task ORDER BY id DESC LIMIT 500;");
  ...
  /**
   * @Route("/api/get/p_task/{projectid}", name="p_task_by_project")
   */
  public function p_task_by_project($projectid) :JsonResponse
  {
    $em = $this->getDoctrine();
    $var = $em->getConnection()->prepare("SELECT *,
                       (SELECT concat(nom, ' ', prenom) FROM p_employe WHERE p_employe.id = p_task.p_employe_id) employe
                       FROM p_task where p_projet_id = ? ORDER BY id DESC LIMIT 500;");
    $res = $var->executeQuery([$projectid])->fetchAll();
    $res = json_decode(json_encode($res, JSON_NUMERIC_CHECK), true);
    return new JsonResponse($res);
  }
