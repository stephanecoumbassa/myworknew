...
class PEmployeController extends BaseController {
    ...
    /**
     * @Route("/api/get/p_employe/{uuid}", name="p_employe_id_get")
     */
    public function p_employe_id_get($uuid) :JsonResponse
    {
        $em = $this->getDoctrine();
        $var = $em->getConnection()->prepare('SELECT * FROM p_employe where euuid = ?;');
        $res = $var->executeQuery([$uuid])->fetch();
        $res = json_decode(json_encode($res, JSON_NUMERIC_CHECK), true);
        return new JsonResponse($res);
    }
