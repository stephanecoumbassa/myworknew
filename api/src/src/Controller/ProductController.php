...
class ProductController extends BaseController
{
    ...
    public function appro_get(Request $request)
    {
        ...
        $var = $em->getConnection()->prepare("SELECT concat(s_product.name, ' ', ifnull(scp.name, ''), ' ', ifnull(sma.nom, '')) prodcat, s_product.id, s_product.name, s_product.name title, disabled,
                                ...
                                sma.nom marque, sma.photo marque_photo, customize, s_product.photo,
...
//        $res = $var;
    ...
    /**
     * @Route("/my/photo/products/{productid}", name="product_post_photo")
     */
    public function product_photo(Request $request, $productid)
    {
        ...
        if( isset($file) && !empty($file) ) {
            ...
            $name = 'product'.uniqid().'.'.$fileExtension;
            $uploadDir = $_SERVER['DOCUMENT_ROOT'].'/apistock/public/assets/uploads/products/';
            $file->move( $uploadDir, $name );
            $em = $this->getDoctrine();
            $var = $em->getConnection()->prepare('UPDATE s_product SET photo = ? WHERE id = ?');
            $var->executeQuery([$name, $productid]);
            return new JsonResponse(['status' =>1, 'msg' => 'Upload OK']);
        }else {
            return new JsonResponse(['status' =>1, 'msg' => 'Fichier vide']);
        }
    }
...
//        return new JsonResponse($data);
        ...
        $params = [
            ...
            $data['hauteur'] ?? 0, $data['poids'] ?? 0, $data['youtube'] ?? null, $data['marque_id'] ?? null, (int)$this->getUser()->getId(), $data['type'] ?? 'produit',
            (int)$this->getUser()->getIdShopId(),
    ...
    public function product_update(Request $request)
    {
        ...
        $params = [
            ...
            (int)$this->getUser()->getId(), (int)$this->getUser()->getIdShopId(), $data['type'] ?? 'produit', (int) $data['id']
        ];
        $var = $em->getConnection()->prepare('UPDATE s_product SET
            ...
            largeur = ?, longueur = ?, poids = ?, hauteur = ?, youtube = ?, marque_id = ?, customize = ?,
                     agent_user = ?, id_magasin = ?, type = ? WHERE id = ?;');
