...
class PProjetController extends BaseController {
    ...
    public function p_projet_get() :JsonResponse
    {
        ...
        $var = $em->getConnection()->prepare("SELECT *,
           (SELECT concat(name, ' ', last_name) from s_clients where s_clients.id = p_projet.clientid) client
           FROM p_projet ORDER BY id DESC LIMIT 500;");
    ...
    /**
     * @Route("/api/get/p_projet/{id}", name="p_projet_id_get")
     */
    public function p_projet_id_get($id) :JsonResponse
    {
        $em = $this->getDoctrine();
        $var = $em->getConnection()->prepare("SELECT *,
                   (SELECT concat(name, ' ', last_name) from s_clients where s_clients.id = p_projet.clientid) client
            FROM p_projet where id = ?;");
    ...
    public function p_projet_search(Request $request) :JsonResponse
    {
        ...
        $var = $em->getConnection()->executeQuery("SELECT *,
               (SELECT concat(name, ' ', last_name) from s_clients where s_clients.id = p_projet.clientid) client
               FROM p_projet WHERE $where", $params);
        $res = $var->fetchAll();
        return new JsonResponse($res);
    }
