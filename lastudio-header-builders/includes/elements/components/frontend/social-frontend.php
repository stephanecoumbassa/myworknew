...
function lahb_social( $atts, $uniqid, $once_run_flag ) {
	...
	if ( $once_run_flag ) :
        ...
        $dynamic_style .= lahb_styling_tab_output( $atts, 'Social Box', '#lastudio-header-builder .social_' . esc_attr( $uniqid ). ' .lastudio-social-icons-box,#header-social-full-wrap-'.esc_attr( $uniqid ).',.main-slide-toggle #header-social-modal' );
        $dynamic_style .= lahb_styling_tab_output( $atts, 'Social Icon/Text Box', '#lastudio-header-builder .social_' . esc_attr( $uniqid ). ' .lastudio-social-icons-box .header-social-icons a, .lastudio-social-icons-box .header-social-icons a, #header-social-modal .header-social-icons a' );
        $dynamic_style .= lahb_styling_tab_output( $atts, 'Social Icon', '#lastudio-header-builder .social_' . esc_attr( $uniqid ). ' .lastudio-social-icons-box .header-social-icons a i, .lastudio-social-icons-box .header-social-icons a i, #header-social-modal .header-social-icons a i','#lastudio-header-builder .social_' . esc_attr( $uniqid ). ' .lastudio-social-icons-box .header-social-icons:hover a i, .lastudio-social-icons-box .header-social-icons:hover a i, #header-social-modal .header-social-icons:hover a i' );
        $dynamic_style .= lahb_styling_tab_output( $atts, 'Social Text', '#lastudio-header-builder .social_' . esc_attr( $uniqid ). ' .lastudio-social-icons-box .header-social-icons a span, .lastudio-social-icons-box .header-social-icons a span, #header-social-modal .header-social-icons a span','#lastudio-header-builder .social_' . esc_attr( $uniqid ). ' .lastudio-social-icons-box .header-social-icons:hover a span, .lastudio-social-icons-box .header-social-icons:hover a span, #header-social-modal .header-social-icons:hover a span' );
        ...
        $dynamic_style .= lahb_styling_tab_output( $atts, 'Full Page Social', 'body.la-social-popup.popup_id_' . esc_attr($uniqid) . ' #lightcase-case' );
	...
	if ( $main_social_icon == 'text' && ! empty( $main_icon_text ) ) {
		$text_icon = '<span>' . LAHB_Helper::render_string($main_icon_text) . '</span>';
	} else {
	...
	// render
	$out .= '
	<div class="lahb-element lahb-icon-wrap lahb-social lahb-social-type-'. esc_attr($social_type) .' ' . esc_attr( $tooltip_class . $extra_class ) . ' lahb-header-dropdown social_'.esc_attr( $uniqid ).'"' . $tooltip . '>';
				...
				<div class="header-social-content container">
					<div class="col-md-6 col-sm-6 col-xs-6">
						<h3 class="header-social-modal-title">
							' . LAHB_Helper::render_string($toggle_text) . '
						</h3>
					</div>
					<div class="col-md-6 col-sm-6 col-xs-6">
...
LAHB_Helper::add_element( 'social', 'lahb_social' );
