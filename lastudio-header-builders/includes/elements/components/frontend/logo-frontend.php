...
function lahb_logo( $atts, $uniqid, $once_run_flag ) {
	...
	$logo_text			= $logo_text ? LAHB_Helper::render_string($logo_text) : get_bloginfo( 'name' );
	...
	if ( $once_run_flag ) :
		...
		$dynamic_style .= lahb_styling_tab_output( $atts, 'Transparent Logo', '.logo_' . esc_attr( $uniqid ) . ' img.lahb-logo-transparent' );
	...
	// render
	$out .= '
	    <div class="lahb-element lahb-logo' . esc_attr( $extra_class ) . ' logo_'.esc_attr( $uniqid ).'"'. ($once_run_flag ? ' itemscope itemtype="http://schema.org/Brand"' : '') .'>
		<a href="' . esc_url( home_url( '/' ) ) . '" ref="home">';
...
LAHB_Helper::add_element( 'logo', 'lahb_logo' );
