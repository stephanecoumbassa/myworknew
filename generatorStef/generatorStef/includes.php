<?php
    ...
    $query = $conn->query("SELECT COLUMN_NAME, COLUMN_TYPE, IS_NULLABLE, COLUMN_KEY, COLUMN_COMMENT FROM information_schema.COLUMNS
    WHERE table_schema = '{$dtb}' AND table_name = '{$tablename}'");
    $res = $query->fetchAll();
    $var = [];
    $vars = [];
