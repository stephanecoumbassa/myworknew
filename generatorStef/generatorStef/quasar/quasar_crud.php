...
foreach ($var as $item) {
    ...
    $shows = $shows."<q-toggle v-model='visibleColumns' val='$item' label='$item' /> \n ";
}
    ...
    <div class="row justify-center">
      <div class="col-12 q-mt-md">
        ...
        <q-table title="{$name}s" :rows="{$name}s" :columns="columns" :filter="filter"
                :visible-columns="visibleColumns" :pagination="pagination" row-key="name">
...
file_put_contents("./files/{$name}/{$name}.vue", $liste.$add.$scripts);
