...
//    $paramsupdate = $paramsupdate.", ".$item." = _".$item;
    ...
    if ( str_contains($item, 'photo') || str_contains($item, 'image') || str_contains($item, 'icon') ) {
        $_filename = $_filename."
        if (!empty({$dollar}request->files->get('$item'))) {
            {$dollar}data['$item'] = {$dollar}this->upload_files('$tablename', {$dollar}request->files->get('$item'), {$dollar}fileUploader);
        }
    ";
//        dump($_filename);
    }
...
<?php
    ...
    public function {$name}_get() :JsonResponse
    {
        ...
        {$dollar}res = json_decode(json_encode({$dollar}res, JSON_NUMERIC_CHECK), true);
    ...
    public function {$name}_search(Request {$dollar}request) :JsonResponse
    {
        ...
        {$dollar}data = json_decode({$dollar}request->getContent(),true) ?? {$dollar}_POST;
     ...
     */
    public function {$name}_add(Request {$dollar}request, FileUploader {$dollar}fileUploader)
    {
        {$dollar}data = json_decode({$dollar}request->getContent(),true) ?? {$dollar}_POST; 
        {$dollar}em = {$dollar}this->getDoctrine();
        $_filename
    ...
    {
        {$dollar}em = {$dollar}this->getDoctrine();
        {$dollar}var = {$dollar}em->getConnection()->prepare('DELETE FROM $name WHERE id = ?;');
...
file_put_contents("./files/_CONTROLLER/{$snake}Controller.php", $class);